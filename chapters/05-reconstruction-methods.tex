...
Since $\nu^2$-Flows processes the full event information to make a prediction for the neutrino momenta, it is agnostic to the jet assignment. This is drastically different from the conventional methods, which were used in previous \atlas analyses \cite{entanglement_top_quarks}. These methods are interdependent with the jet assignment and mostly based on exploiting the kinematic constraints of the event, such as the invariant masses of the $W$ bosons and top quarks outlined in \Cref{sec:kinematic_constraints}.
