...
In the context of this thesis, machine learning refers to a subset of artificial intelligence that involves the development of algorithms and statistical models that enable computers to perform specific tasks without explicit instructions. Instead, these algorithms learn patterns and relationships from data to make predictions or decisions.\\
...
The training process involves iteratively updating (called \textit{epoch}) the model's parameters using an optimization algorithm to minimize the loss function.
...
\begin{figure}[h]
    ...
    \caption{Schematic of a single artificial neuron (perceptron). The neuron receives multiple input signals, each associated with a weight, and computes a weighted sum of these inputs, optionally a so-called bias is a added. An activation function is then applied to this sum to produce the neuron's output.}
    \label{fig:perceptron}
\end{figure}
The simplest unit of a neural network is the artificial neuron, also known as a perceptron \cite{perceptron}. A perceptron takes multiple input signals, each associated with a weight, and computes a weighted sum of these inputs. An activation function is then applied to this sum to produce the neuron's output. This structure is illustrated in Figure \ref{fig:perceptron}. The general structure of a perceptron is inspired by the biological neurons. The activation function introduces non-linearity into the model, allowing it to learn complex patterns in the data. Common activation functions include the sigmoid function, hyperbolic tangent (tanh), and rectified linear unit (ReLU) \cite{relu}.


\subsection{Dense Neural Network}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/machine-learning/dnn.pdf}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/machine-learning/rnn.pdf}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[]{figures/machine-learning/dpa.pdf}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[]{figures/machine-learning/mha.pdf}
        \caption{Multi-Head Attention}
        \label{fig:mha}
    \end{subfigure}
    \caption{Schematics of attention mechanisms. (a) Scaled Dot Product Attention computes attention scores between query and key vectors, scales them, applies a softmax function to obtain attention weights, and uses these weights to compute a weighted sum of the value vectors. (b) Multi-Head Attention consists of multiple parallel attention heads, each performing scaled dot product attention on linearly transformed versions of the input queries, keys, and values. The outputs of all heads are concatenated and linearly transformed to produce the final output.}
    \label{fig:attention}
\end{figure}
The transformer architecture deals with sequential data, similar to RNNs, but it does so using a mechanism attention rather than recurrent connections. This allows transformer to process information in parallel, making them more efficient for training on large datasets. Additionally, transformer can capture long-range dependencies in the data more effectively than RNNs, as they relate all elements of the input sequence to each other through attention mechanisms.\\
The core component of the attention mechanism is the \textit{multi-head attention} (MHA) \cite{attention_is_all_you_need}, which allows the model to focus on different parts of the input sequence simultaneously. A schematic of the MHA mechanism is shown in Figure \ref{fig:mha}.\\